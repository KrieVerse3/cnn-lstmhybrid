...
def predict(model, df, scaler, features, date):
    try:
        ...
        if date > df.index[-1]:
            last_idx = len(df) - 1
            months_missing = (date.year - df.index[-1].year) * 12 + (date.month - df.index[-1].month)
            print(f"Warning: Projecting {months_missing} months into future")
        else:
            last_idx = df.index.get_loc(date)

        # Create sequence with enhanced trend and seasonality
        seq = []
        for i in range(lookback, 0, -1):
            idx = last_idx - i
            if idx >= 0:
                point = df.iloc[idx][features].copy()
            else:
                # Enhanced synthetic data generation
                point = df.iloc[0][features].copy()

                # Apply linear trend based on feature's historical trend
                monthly_trend = df[features].diff(12).mean() / 12
                point += monthly_trend * (-idx)

                # Apply seasonality
                if date.month in df.index.month:
                    seasonal_adjustment = df[df.index.month == date.month][features].mean() - df[features].mean()
                    point += seasonal_adjustment
